...
select substr('DBAII 2023 Unifranz',1); # le cortamos de donde queramos siepre uno despues
...
select cadenanumero('DBAII',3);
 ...
 create or replace function contadorDeLetras(Cadena varchar(100),letra char)
returns text
Begin
    declare respuesta text default 'La letra ni existe en la cadena';
    declare cont int default 1;
    declare nVeces int default 0;
    declare puntero char;
    if locate(letra,Cadena)>0 Then
        while(cont <= char_length(Cadena)) do #char_length(Cadena) conrtador de la cadena
...
select contadorDeLetras('hola hueon','h');

 create or replace function contadordeVocales(Cadena varchar(100))
returns text
Begin
    declare respuesta text default 'La letra ni existe en la cadena';
    declare cont int default 1;
    ...
    if locate('a',Cadena)>0 or locate('e',Cadena)>0 or locate('i',Cadena)>0 or locate('o',Cadena)>0 or locate('u',Cadena)>0 Then
        ...
        set respuesta= concat(' las vocales se repiten :', nVeces);
    end if;
    return  respuesta;

end;
select contadordeVocales('HLAAAAAAAAAAAAAAA');
...
Begin
    ...
    if locate('a',Cadena)>0 or locate('e',Cadena)>0 or locate('i',Cadena)>0 or locate('o',Cadena)>0 or locate('u',Cadena)>0 Then
        while(cont <= char_length(Cadena)) do #char_length(Cadena) conrtador de la cadena
        if(substr(Cadena,cont,1) like '%aeiou%') then
        set nVeces =nVeces +1;
        end if;
        set cont=cont+1;
        end while;
...
select contadordeVocales('HLAAAAAAAAAAAAAAA');
 ...
 create or replace function contadordePalabras(Cadena varchar(100))
returns text
Begin
    declare respuesta text default 'La letra ni existe en la cadena';
    declare cont int default 1;
...
select contadordePalabras('nilver mayta Cuno');

 create or replace function cortadorDeNombre(Cadena varchar(100))
returns text
Begin
    declare respuesta text default 'La letra ni existe en la cadena';
    ...
    declare puntero char;
        ...
        if nVeces = 1 then
            set respuesta = substr(Cadena,cont+2);
        end if;
        set cont=cont+1;
        end while;
    return  respuesta;
...
select cortadorDeNombre('Luis Alvarez MEdina')
...
Begin
    ...
    return  substr(Cadena,Posicion+1);
